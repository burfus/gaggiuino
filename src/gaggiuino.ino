...
static void brewDetect(void) {
  ...
  if (currentState.brewSwitchState && !stoppedOnWeight && millis() >= brewDebounceTimer) {
    if(!paramsReset) {
      currentState.shotWeight = 0.f;
      brewParamsReset();
      paramsReset = true;
    ...
    if(paramsReset) {
      brewParamsReset();
      paramsReset = false;
      if(scalesIsPresent) currentState.shotWeight = currentState.shotWeight + currentState.smoothedWeightFlow / 2.f;
    }
  }
}
